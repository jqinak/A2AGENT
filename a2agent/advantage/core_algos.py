...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_est(AdvantageEstimator.A2PO)  # or simply: @register_adv_est("grpo")
...
#     response_mask = torch.tensor([
#     [1, 1, 1, 1, 0, 0],  # 响应1: 4个有效token
#     [1, 1, 1, 0, 0, 0],  # 响应2: 3个有效token
#     [1, 1, 1, 1, 1, 0],  # 响应3: 5个有效token
#     [1, 1, 1, 1, 1, 1]   # 响应4: 6个有效token
# ])
    """
    grpo在这里时根据token计算样本优势的
        a2po的任务场景:

        <think></think><answer></answer><code><code>
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                ...
                scores[i] = scores[i] - id2mean[index[i]]
         # (batch_size, 1) -> (batch_size, response_length)
        scores = scores.unsqueeze(-1) * response_mask
