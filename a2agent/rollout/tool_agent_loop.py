...
from a2agent.rollout.agent_loop import (
    AgentLoopBase,
    AgentLoopOutput,
...
from a2agent.tools.tool_parser import FunctionCall, ToolParser
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
            ...
            if tool_response.video:
                ...
                if isinstance(tool_response.video, list):
                    ...
                    for vid in tool_response.video:
                        if vid is not None:  # Add a check to ensure the image is not None
                            new_videos_this_turn.append(vid)  # Using local variable
                else:
                    # Ensure the image is not None
                    if tool_response.video is not None:
                        new_videos_this_turn.append(tool_response.video)  # Using local variable
        ...
        if new_videos_this_turn:
            ...
            for img in new_videos_this_turn:
                agent_data.video_data.append(img)
