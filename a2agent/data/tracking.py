...
if __name__ == "__main__":
    ...
    parser.add_argument("--local_dataset_path", default='/nvme/data-pool1/qjl/A2VL_DATA/data/tracking', help="The local path to the raw dataset, if it exists.")
    parser.add_argument(
        "--local_save_dir",
        default="/home/jincai_guo/ICML2025_JIE/QJL/A2AGENT/a2agent/data/tracking",
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            video = str(example.pop("videos")[0])
            video = str(example.pop("videos")[0])
            if video.startswith("/nvme/data-pool1/qjl/OneThinker-train-data/"):
                video = video.replace("/nvme/data-pool1/qjl/OneThinker-train-data/","")
            ...
            remove_text = "Provide your thinking process between the <think> and </think> tags, and then give your final answer between the <answer> and </answer> tags.At the end, you must output the final answer in the format:\n<answer><your_answer_here></answer>\nPlease track the target object throughout the video and provide one bounding box per second, ONLY up to 32 seconds, within the <answer>...</answer> tags.\nExample:\n<answer>{\"boxes\": {\"1\": [405, 230, 654, 463], \"2\": [435, 223, 678, 446], ..., \"32\": [415, 203, 691, 487]}}</answer>\nNote: Each key in 'boxes' must correspond to a second (1, 2, 3, ..., 32) and contain a 4-number bounding box [x1, y1, x2, y2]."
            to_text = "Please track the target object throughout the video and provide one bounding box per second, ONLY up to 32 seconds."
            ...
            data = {
                ...
                "videos": [{
                    "type": "video",
                    "video": f"/nvme/data-pool1/qjl/A2VL_DATA/{video}",
                    "fps": 2,
                    "min_frames": 1,
                    "max_frames": 128
                }],
